...
class BitaminHistoryReceive extends PureComponent {
    state = {
        offsetNumber: 0,
        fooOffset: true,
        historyDataUpdate: []
    ...
    componentDidMount() {
        ...
        if (history && history.length !== 0 && history[0].category === 'RECEIVE') {
            this.setState({
        ...
        if (history) {
            if (!isEqual(history, prevProps.history || [])) {
                if (history.length > 0 && history[0].category === requestCategory) {
                    ...
                    this.setState({historyData: historyDataUpdate});
                }
            }
        }
    ...
    loadingAfterScroll = (e) => {
        ...
        if (bottom && history.length !== 0) {
            ...
            this.props.getBitaminHistory({userId: _id, category: "RECEIVE", offsetNumber: offsetNumber + 1});
        }
    };
    loadingFirst = () => {
        return (
    ...
    render() {
        const {historyData , offsetNumber} = this.state;
        const {history} = this.props;
        return (
            <Fragment>
                                ...
                                return (
                                    <div key={i}>
                                        <div className={'history-item-wrapper'}>
                                            ...
                                            <span className='detail'><TranslateLanguage
                                                direct={'bitamin.bitaminHistory.getDetail.profit'}/></span>
                                            </div>
...
const mapStateToProps = (state) => {
    const {authentication: {user}, bitaminReducer: {requestCategory, loading}} = state;
    return {user, requestCategory, loading}
};

const mapDispatchToProps = (dispatch) => ({
    getBitaminHistory: (param) => dispatch(bitaminActions.getBitaminHistory(param))
});
export default connect(mapStateToProps, mapDispatchToProps)(BitaminHistoryReceive)