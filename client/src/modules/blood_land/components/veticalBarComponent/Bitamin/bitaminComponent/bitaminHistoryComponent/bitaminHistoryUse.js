...
class BitaminHistoryUse extends PureComponent {
    state = {
        offsetNumber: 0,
        fooOffset: true,
        historyDataUpdate: []
    ...
    componentDidMount() {
        ...
        if(history && history.length !== 0 && history[0].category === 'USE'){
            const {historyData} = this.state;
            if (!isEqual(history, historyData)) {
                this.setState({
                    historyData: history ? history.filter(i => i.status === true) : [],
                    isBottom : false
                })
            }
        }
        ...
        if(history && history.length !== 0 && history[0].category === 'USE'){
            ...
            if(!isEqual(history, prevProps.history || [])){
                ...
                if(history.length > 0 && history[0].category === requestCategory){
                    ...
                    this.setState({historyData: history ? historyDataUpdate : [], isBottom : false});
                }
            }
        }
    ...
    loadingAfterScroll = (e) => {
        ...
        if (bottom && history.length !== 0) {
            ...
            this.props.getBitaminHistory({userId: _id, category: "USE", offsetNumber: offsetNumber + 1});
        }
    };
    loadingFirst = () => {
        return (
    ...
    render() {
        const {historyData , offsetNumber , isBottom} = this.state;
        const {history} = this.props;
...
export default connect(mapStateToProps, mapDispatchToProps)(BitaminHistoryUse)